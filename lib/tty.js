...
function Session(server, socket) {
  ...
  if(conf.keyKeeper) {
    this.key = conf.keyKeeper.getKey(req.headers.cookie);
    logger.log('session key:' + this.key);
  };
...
Session.prototype.handleCreate = function(cols, rows, func) {
  ...
  if(conf.keyKeeper){
    shell = conf.keyKeeper.shell(key);
    shellArgs = conf.keyKeeper.shellArgs(key);
  } else {
    shell = conf.shell;
    shellArgs = conf.shellArgs;
  }
